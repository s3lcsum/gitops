...
- [x] (retroactively added) Home Assistant route in the Traefik file provider (host-network container is invisible to the Docker provider)
...
**Home Assistant was 404 on the tunnel.** `hass` is `network_mode: host`, and Portainer Traefik only attaches containers on the `proxy` network, so the compose labels never became a router. `https://hass.dominiksiejak.pl` came back 404 from Germany and Spain while `n8n` `/healthz` was 200 and Authentik was 302 on the same `https://traefik` origin. The route now lives in `stacks/traefik/dynamic.yaml` (`host.docker.internal:8123`, `remote` middleware). It goes live on the next Portainer sync, then restart the `traefik` container so the mounted file is picked up.
