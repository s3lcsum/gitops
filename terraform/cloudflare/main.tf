  ...
  rules = [
    {
      ...
      action      = "skip"
      expression  = "(http.host eq \"n8n.dominiksiejak.pl\" and (starts_with(http.request.uri.path, \"/webhook/messenger\") or starts_with(http.request.uri.path, \"/webhook/toggl-time-entry\") or starts_with(http.request.uri.path, \"/webhook/focus-work\") or starts_with(http.request.uri.path, \"/webhook/authentik-login\"))) or http.host in {\"dominiksiejak.pl\" \"www.dominiksiejak.pl\" \"miedzysztuka.dominiksiejak.pl\" \"polibiurokc.dominiksiejak.pl\"}"
